...
app.delete('/books/:id', (req, res) => {
    if(ObjectId.isValid(req.params.id)){
        db.collection('books')
        .deleteOne({_id : new ObjectId(req.params.id)}) // returns a cursor pointing to all the documents
        .then(result => {
...
app.patch('/books/:id', (req, res) => {
    ...
    if(ObjectId.isValid(req.params.id)){
        db.collection('books')
        .updateOne({_id : new ObjectId(req.params.id)}, {$set: updates}) // returns a cursor pointing to all the documents
        .then(result => {
            res.status(200).json(result);
        })
        .catch(err => {
